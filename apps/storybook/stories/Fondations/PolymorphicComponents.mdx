...
```tsx
// ✅ With polymorphism - one component, multiple uses
...
```tsx
// ✅ Correct - TypeScript knows about href, target, rel
<Button as="a" href="/home" target="_blank" rel="noopener noreferrer">
  Link
</Button>

// ❌ Error - TypeScript knows 'href' is not valid for buttons
<Button href="/home">Link</Button>

// ❌ Error - TypeScript knows 'onClick' is not valid for links... wait, it is!
...
// ✅ TypeScript knows about React Router's 'to' prop
<Button as={Link} to="/dashboard">
  Dashboard
</Button>

// ❌ Error - 'to' requires 'as={Link}'
<Button to="/dashboard">
  Dashboard
</Button>
```
...
2. **Don't break semantic meaning**
   ...
   ```tsx
   // ❌ Bad - div cannot be in tab order properly
   <Button as="div" onClick={handleClick}>Click</Button>

   // ✅ Better - use default button
   <Button onClick={handleClick}>Click</Button>
   ```
...
3. **Don't mix incompatible behaviors**
   ...
   ```tsx
   // ❌ Bad - links shouldn't have onClick without href
...
```tsx
// ✅ All ARIA attributes work correctly
...
✅ **No boilerplate** - The type handles all TypeScript complexity\
✅ **Full type inference** - Automatically infers props based on the `as` prop\
✅ **Ref support** - Built-in ref forwarding support\
✅ **Consistent API** - Same pattern as all ShelterUI components\
...
✅ **Reduce code duplication** - One component, many uses\
✅ **Maintain consistency** - Same styling and behavior everywhere\
✅ **Improve type safety** - Full TypeScript inference\
✅ **Preserve accessibility** - Correct semantics and ARIA support\
✅ **Enable flexibility** - Integrate with any framework or library
...
> 🎯 **Key Takeaway**:
> The `as` prop transforms a single component into a Swiss Army knife of UI building—use the right tool for the right job, with the guarantee of consistency and accessibility.
